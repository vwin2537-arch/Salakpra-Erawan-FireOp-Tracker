...
export const HistoryList: React.FC<HistoryListProps> = ({ activities, onEdit, onDelete, settings }) => {
    ...
    return (
        <div className="p-4 md:p-8 min-h-screen relative font-sans pb-24 md:pb-10">

            {/* --- HEADER --- */}
            <div className="mb-6 flex flex-col gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-white tracking-tight flex items-center gap-2">
                        <Clock className="text-orange-500" /> ACTIVITY TIMELINE
                    </h2>
                    <p className="text-slate-400 text-sm">ประวัติการดำเนินงานทั้งหมด ({baseFiltered.length})</p>
                </div>

                <div className="flex flex-col md:flex-row gap-3">
                    <div className="relative flex-1 group">
                        <Search className="absolute left-3 top-2.5 text-slate-500 group-focus-within:text-orange-500 transition-colors" size={18} />
                        <input
                            type="text"
                            placeholder="ค้นหากิจกรรม..."
                            className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-600 focus:outline-none focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500 bg-slate-800/50 text-white placeholder-slate-500 shadow-sm transition-all"
                            value={searchTerm}
                    ...
                    <div className="relative">
                        <select
                            className="w-full h-full pl-4 pr-10 py-2.5 rounded-xl border border-slate-600 focus:outline-none focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500 appearance-none bg-slate-800/50 text-slate-300 text-sm font-medium shadow-sm cursor-pointer"
                            value={filterPhase}
                        ...
                        <Filter className="absolute right-3 top-3 text-slate-500 pointer-events-none" size={14} />
                    </div>
                </div>
            </div>
            ...
            {chartData.length > 0 && (
                <div className="mb-8 p-5 rounded-2xl border" style={{ background: 'rgba(30, 41, 59, 0.8)', borderColor: 'rgba(71, 85, 105, 0.3)' }}>
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2 text-white font-bold text-sm">
                            <BarChart2 size={18} className="text-orange-500" />
                            <span>MISSION SUMMARY (Click to Filter)</span>
                        </div>
                        {filterCategory && (
                            <button
            ...
            <div className="relative space-y-8 pl-4 md:pl-0">
                ...
                {filterCategory && (
                    <div className="flex items-center justify-between bg-orange-500/20 border border-orange-500/30 px-4 py-2 rounded-lg mb-4 text-sm text-orange-300 animate-fade-in">
                        <div className="flex items-center gap-2 font-bold">
                            <ListFilter size={16} />
                            Showing Category: {getCategoryLabel(filterCategory)}
                        </div>
                        <button onClick={() => setFilterCategory(null)} className="p-1 hover:bg-orange-500/30 rounded-full">
                            <X size={16} />
                        </button>
                    </div>
                )}
                ...
                {sorted.map((activity, index) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            {/* Card */}
                            <div className="flex-1 rounded-2xl p-4 md:p-5 border hover:border-orange-500/50 transition-all" style={{ background: 'rgba(30, 41, 59, 0.8)', borderColor: 'rgba(71, 85, 105, 0.3)' }}>
                                ...
                                <div className="flex justify-between items-start gap-4 mb-2">
                                    <div className="flex-1">
                                        ...
                                        <div className="flex items-center gap-2 mb-1">
                                            <span className="px-2 py-0.5 rounded-md bg-slate-700 text-slate-300 text-[10px] font-bold uppercase tracking-wide">
                                                {getCategoryLabel(activity.category)}
                                            </span>
                                        </div>
                                        <h3 className="text-base md:text-lg font-bold text-white leading-snug group-hover:text-orange-400 transition-colors">
                                            {activity.title}
                                        </h3>
                                    </div>
                                    {/* Desktop Actions */}
                                    <div className="hidden md:flex gap-1">
                                        <button onClick={(e) => handleEditClick(e, activity)} className="p-1.5 text-slate-500 hover:text-indigo-400 rounded hover:bg-indigo-500/20"><Pencil size={16} /></button>
                                        <button onClick={(e) => handleDeleteClick(e, activity.id)} className="p-1.5 text-slate-500 hover:text-red-400 rounded hover:bg-red-500/20"><Trash2 size={16} /></button>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center gap-4 text-xs text-slate-500 border-t border-slate-700/50 pt-3">
                                    {activity.stats?.areaDamaged && (
                                        <span className="flex items-center gap-1 text-red-400"><span className="font-bold">{activity.stats.areaDamaged}</span> ไร่</span>
                                    )}
                                    {activity.stats?.personnelCount && (
                                        <span className="flex items-center gap-1 text-blue-400"><span className="font-bold">{activity.stats.personnelCount}</span> นาย</span>
                                    )}
                                    {activity.location?.utm && (
                                        <span className="flex items-center gap-1 ml-auto text-slate-500"><MapPin size={12} /> {activity.location.utm}</span>
                                    )}
                                </div>
                            </div>
                        </div>
                ...
                {sorted.length === 0 && (
                    <div className="text-center py-20">
                        <div className="inline-flex p-4 rounded-full bg-slate-800 text-slate-500 mb-3">
                            <Search size={32} />
                        </div>
                        <p className="text-slate-300 font-medium">ไม่พบข้อมูลกิจกรรม</p>
                        <p className="text-slate-500 text-sm">ลองเปลี่ยนคำค้นหาหรือตัวกรอง</p>
                    </div>
                )}
            </div>
